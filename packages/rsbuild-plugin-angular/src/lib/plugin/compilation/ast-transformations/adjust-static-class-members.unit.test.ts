...
function testCase({
  ...
  return async () => {
    const result = adjustStaticClassMembers(input, options);
    if (!result) {
      assert.fail(
        'Expected adjustStaticClassMembers to return a transformed result.'
      );
    } else {
      const formattedResult = await format(result);
      const formattedExpected = await format(
        expected === NO_CHANGE ? input : expected
      );
...
describe('adjust-static-class-members AST Transformations', () => {
  it(
  ...
  it(
    'does not wrap default exported class with only side effect fields',
    testCase({
      input: `
      export default class CustomComponentEffects {
        constructor(_actions) {
      ...
      expected: `
        let CommonModule = /*#__PURE__*/ (() => {
          class CommonModule {
            ...
            static {
              this.ɵmod = ɵngcc0.ɵɵdefineNgModule({ type: CommonModule });
            }
          }
    ...
    testCase({
      ...
      expected: `
        let CommonModule = /*#__PURE__*/ (() => {
          ...
          CommonModule.ɵinj = /*@__PURE__*/ ɵngcc0.ɵɵdefineInjector({ providers: [{ provide: NgLocalization, useClass: NgLocaleLocalization }]});
          return CommonModule;
        })();
      `,
    })

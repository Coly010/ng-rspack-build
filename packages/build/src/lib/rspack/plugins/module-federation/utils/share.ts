...
import {
  readTsConfigPaths,
  getRootTsConfigPath,
} from '@nx/js/src/utils/typescript/ts-config';
import {
...
export function shareWorkspaceLibraries(
  ...
  const tsconfigPathAliases = readTsConfigPaths(tsConfigPath);
...
export function sharePackages(
  ...
  packages.forEach((pkg) => {
    const pkgVersion =
      pkgJson.dependencies?.[pkg] ?? pkgJson.devDependencies?.[pkg] ?? '0.0.1';

    ...
    if (moduleFederationManifestJson) {
      ...
      if (
        Object.keys(parsedManifest).every(
          (key) => typeof parsedManifest[key] === 'string'
        )
      ) {
...
export function getRemotes(
  ...
  logger.info(
    `NX Starting module federation dev-server for ${context.projectName} with ${
      [...knownRemotes, ...knownDynamicRemotes].length
    } remotes`
  );
...
export function getModuleFederationConfig(
  ...
  try {
    ...
    throw new Error(
      `Could not load ${moduleFederationConfigPath}. Was this project generated with "${pluginName}:host"?`
    );
  }
}
